...
function Tests() {
  ...
  const SeparateLine = () => <hr className={styles["separate-line"]} />;
    ...
    return (
      <div className={styles["test-answers"]}>
        {test.answers !== undefined &&
          test.answers.map((currentObj, index) => {
            return (
              <div key={Math.random() * 1000}>
                <div className={styles["test-answer"]}>
                  <Checkbox
                    obj={currentAnswer[index]}
                    onChange={() => {
                ...
                {test.answers.length - 1 !== index && <SeparateLine />}
              </div>
            );
          })}
      </div>
  ...
  return (
    <div className={styles.main}>
      ...
      <TestFooter
        onNext={onNext}
        onSubmit={onSubmit}
        isSubmitLightsUp={isSubmitLightsUp}
